...
import { assertCanReadReports, getOccupancyReport, parseReportDateRange } from '../../../../lib/server/reporting';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await getOccupancyReport(actor, filters);
    return NextResponse.json({ data, filters });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed to load occupancy report.';
